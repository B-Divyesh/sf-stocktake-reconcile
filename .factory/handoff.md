# Handoff — Stocktake Reconcile v0.1.4
...
After CI passes, tag and push `v0.1.4`; `.github/workflows/release.yml` builds and publishes the release. The release job generates `SHA256SUMS` and `latest.json`; the landing page uses GitHub's CORS-enabled public release API to select the matching platform asset while the release contains `latest.json` for the install scripts.
